...
基于 **Electron + Monaco Editor** 的轻量级代码编辑器，面向 Java 项目的日常浏览与编辑场景，提供文件树、符号大纲、全局搜索、GBK/UTF-8 编码自动识别等能力，并内置 SQL / Markdown 增强高亮与 Markdown 实时预览。
...
└── renderer/               # 渲染进程
    ...
    ├── index.html          # 页面骨架（活动栏/侧边栏/编辑区/状态栏）
    ├── components/         # EditorPane / FileTree / OutlinePanel /
    │                       # QuickOpen / SearchPanel / SettingsPanel /
    │                       # MarkdownPreview（Markdown 渲染预览）
    ├── services/           # java-language（高亮+折叠）、sql-language、
    │                       # markdown-language、icons（文件图标）
...
- `Ink Java Editor Setup x.y.z.exe` — NSIS 安装包
- `Ink Java Editor x.y.z.exe` — 免安装便携版
